...
int main(int argc, char **argv) {
    ...
    std::cout<<"Tareas asíncronas"<<std::endl;
    std::future<Image<unsigned char>> future_srm3x3 =
        std::async(std::launch::async, compute_srm, std::cref(image), 3);
    
    std::future<Image<unsigned char>> future_srm5x5 =
        std::async(std::launch::async, compute_srm, std::cref(image), 5);

    std::future<Image<unsigned char>> future_ela =
        std::async(std::launch::async, compute_ela, std::cref(image), 90);

    std::future<Image<unsigned char>> future_dct_invert =
        std::async(std::launch::async, compute_dct, std::cref(image), block_size, true);

    std::future<Image<unsigned char>> future_dct_direct =
        std::async(std::launch::async, compute_dct, std::cref(image), block_size, false);
    
    Image<unsigned char> srm3x3 = future_srm3x3.get();
    Image<unsigned char> srm5x5 = future_srm5x5.get();
    Image<unsigned char> ela = future_ela.get();
    Image<unsigned char> dct_invert = future_dct_invert.get();
    Image<unsigned char> dct_direct = future_dct_direct.get();
    save_to_file("async_srm_kernel_3x3.png", srm3x3);
    save_to_file("async_srm_kernel_5x5.png", srm5x5);
    save_to_file("async_ela.png", ela);
    save_to_file("async_dct_invert.png", dct_invert);
    save_to_file("async_dct_direct.png", dct_direct);
    auto end_total = std::chrono::steady_clock::now();

    std::cout<<"Tiempo total de async: "
