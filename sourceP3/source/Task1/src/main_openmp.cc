...
Image<unsigned char> compute_dct(const Image<unsigned char> &image, int block_size, bool invert) {
    auto begin = std::chrono::steady_clock::now();
    std::cout<<"Computing DCT "; 
    if (invert) std::cout<<" inverse";
    else std::cout<<" direct";
    std::cout<<" DCT "<<block_size<<"x"<<block_size<<" with OpenMP..."<<std::endl;
    ...
    auto end = std::chrono::steady_clock::now();
    std::cout<<"DCT OpenMP elapsed time: "
...
int main(int argc, char **argv) {
    ...
    std::cout<<"Máximo de hilps openMP: "<<omp_get_max_threads()<<std::endl;
